...
* Unfortunately, when analyzing to get the effective neff(z) for Y3 and Y6, I discovered a bug in the Y1 and Y10 neff(z) from the DESC SRD (!).  The overall normalization of neff was not affected, just the redshift distribution. For now I have results for Y3 and Y6 without bugs, since I identified and fixed the bug when producing them.  They cannot be combined with the buggy Y1 and Y10 results in a sensible way, unfortunately.  Later this week I should self-consistently recalculate them all (Y1 and Y10 as well, which I had avoided simulating due to limited computing resources) and record this bug in the Requirements repo.  However, for now I think we should simply take the DESC SRD v1 Y1 and Y10 alpha and z0 with a linear fit to interpolate to intermediate values:
